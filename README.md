...
- "**Save individual unfiltered 1:1 pixel images**". This option saves a separate TIFF image for each imzML file, for each lipid species. These images are unfiltered, and not scaled (=each pixel in the imzML file is mapped directly to a pixel in the exported image without scaling). If pixel size metadata is available, it is written into the TIFF metadata.
